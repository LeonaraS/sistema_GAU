            ...
            <div class="list-group-item">
                <div class="d-flex">
                    <div class="mr-auto p-2">
                        <h2 class="display-4 titulo">Listar Árvores</h2>
                    </div>
                </div>
                                ...
                                try {
                                    $arvores = Arvore::listar();
                                        foreach ($arvores as $arvore) {								
                                            echo '<tr>';
                                                echo '<td>'.$arvore->getIdarvore().'</td>';
                                                echo '<td>'.$arvore->getAreaverdeIdareaverde().'</td>';
                                                echo '<td>'.$arvore->getFuncionarioIdFuncionario().'</td>';
                                                echo '<td>'.$arvore->getMatriz().'</td>';											
                                                echo '<td>'.$arvore->getAltura().'</td>';											
                                                echo '<td>'.$arvore->getLargura().'</td>';											
                                                echo '<td>'.date("d-m-Y", strtotime($arvore->getDataPoda())).'</td>';											
                                                echo '<td>'.$arvore->getEliminacao().'</td>';											
                                                echo '<td>'.$arvore->getFitossanidade().'</td>';											
                                                echo '<td class="text_wrap">'.$arvore->getObservacao().'</td>';	
                                                echo '<td><a class="mr-2 btn btn-outline-warning btn-sm" href="./editar.php?&id='.$arvore->getIdarvore().'">Editar</a>';	
                                                echo '<a class="btn btn-outline-danger btn-sm" href="./excluir.php?&id='.$arvore->getIdarvore().'">Excluir</a></td>';
                                            echo '</tr>';							
                                        }				
                                    } catch (Exception $e) {
