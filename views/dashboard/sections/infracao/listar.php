            ...
            <div class="list-group-item">
                <div class="d-flex">
                    <div class="mr-auto p-2">
                        <h2 class="display-4 titulo">Listar Infrações</h2>
                    </div>
                </div>
                                ...
                                try {
                                    $infracoes = Infracao::listar();
                                        foreach ($infracoes as $infracao) {								
                                            echo '<tr><td>'.$infracao->getIdinfracao().'</td>';
                                            echo '<td>'.$infracao->getFuncionarioIdfuncionario().'</td>';
                                            echo '<td>'.$infracao->getNomeRazao().'</td>';
                                            echo '<td>'.$infracao->getCpfCnpj().'</td>';											
                                            echo '<td>'.$infracao->getFone().'</td>';											
                                            echo '<td>'.$infracao->getEmail().'</td>';											
                                            echo '<td>'.date("d-m-Y", strtotime($infracao->getDataInfracao())).'</td>';											
                                            echo '<td class="txt_rap">'.$infracao->getObservacao().'</td>';	
                                            echo '<td><a class="mr-2 btn btn-outline-warning btn-sm" href="./editar.php?&id='.$infracao->getIdinfracao().'">Editar</a>';	
                                            echo '<a class="btn btn-outline-danger btn-sm" href="./excluir.php?&id='.$infracao->getIdinfracao().'">Excluir</a></td></tr>';						
                                        }				
                                    } catch (Exception $e) {
