            ...
            <div class="list-group-item">
                <div class="d-flex">
                    <div class="mr-auto p-2">
                        <h2 class="display-4 titulo">Listar Requerimento</h2>
                    </div>
                </div>
                                ...
                                try {
                                    $requerimentos = Requerimento::listar();
                                        foreach ($requerimentos as $requerimento) {								
                                            echo '<tr><td>'.$requerimento->getIdrequerimento().'</td>';
                                            echo '<td>'.$requerimento->getFuncionarioIdfuncionario().'</td>';
                                            echo '<td>'.$requerimento->getCidadaoIdcidadao().'</td>';
                                            echo '<td>'.$requerimento->getCopiaRg().'</td>';
                                            echo '<td>'.$requerimento->getCopiaCpf().'</td>';
                                            echo '<td>'.$requerimento->getCompResidencia().'</td>';
                                            echo '<td>'.$requerimento->getAutorizacao().'</td>';
                                            echo '<td>'.date("d-m-Y", strtotime($requerimento->getDataReqto())).'</td>';	
                                            echo '<td><a class="mr-2 btn btn-outline-warning btn-sm" href="./editar.php?&id='.$requerimento->getIdrequerimento().'">Editar</a>';	
                                            echo '<a class="btn btn-outline-danger btn-sm" href="./excluir.php?&id='.$requerimento->getIdrequerimento().'">Excluir</a></td></tr>';					
                                        }				
                                    } catch (Exception $e) {
